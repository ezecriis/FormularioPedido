//	Formulario de pago de una tiena online simple usando: HTML, CSS, JS
...
window.onload = function () {
	...
	// Se cargan los productos dentro del HTML de forna dinamica haciendo uso de los datos de la base de datos, como si de un PHP se tratara:
	...
	// Rellena el campo dia y año, de la fecha y tarjeta de credito:
	// Fecha entrega
	var fecha = new Date();
	var anio = fecha.getFullYear();

	for (var i = 1; i <= 31; i++) {
		document.getElementById("fechaEntrega").innerHTML = document.getElementById("fechaEntrega").innerHTML + '<option value="' + i + '">' + i + '</option>';
	}

	for (var i = anio; i >= (2020); i--) {
		document.getElementById("fechaEntregaAnio").innerHTML = document.getElementById("fechaEntregaAnio").innerHTML + '<option value="' + i + '">' + i + '</option>';
	}
...
function validaLasUnidades(elEvento) {
	...
	// Si no ha habido un error, se ejecuta la siguiente funcion que se encarga de cargar el carro de la compra:
	if (todoBien) {
		calculaElTotal();
	}
}
...
function validaDatosPersonales(elEvento) {
	...
	// Fecha de entrega DIA:
	var vfechaEntrega = document.getElementById("fechaEntrega").selectedIndex;
	if (vfechaEntrega == null || vfechaEntrega == 0) {
		todoBien = false;
		document.getElementById("fechaEntrega").className = "textMal";
	}
	else {
		document.getElementById("fechaEntrega").className = "textBien";
	}
	// Fecha de nacimiento MES:
	var vfechaEntregaMes = document.getElementById("fechaEntregaMes").selectedIndex;
	if (vfechaEntregaMes == null || vfechaEntregaMes == 0) {
		todoBien = false;
		document.getElementById("fechaEntregaMes").className = "textMal";
	}
	else {
		document.getElementById("fechaEntregaMes").className = "textBien";
	}
	// Fecha de nacimiento AÑO:
	var vfechaEntregaAnio = document.getElementById("fechaEntregaAnio").selectedIndex;
	if (vfechaEntregaAnio == null || vfechaEntregaAnio == 0) {
		todoBien = false;
		document.getElementById("fechaEntregaAnio").className = "textMal";
	}
	else {
		document.getElementById("fechaEntregaAnio").className = "textBien";
	}
	...
	// Calle nombre:
	var vCalleNombre = document.getElementById("calleNombre").value;
	if (vCalleNombre == null || vCalleNombre.length == 0 || /^\s+$/.test(vCalleNombre) || !isNaN(vCalleNombre)) {
		todoBien = false;
		document.getElementById("calleNombre").className = "textMal";
	}
	else {
		document.getElementById("calleNombre").className = "textBien";
	}


	// Calle Numero:
	var vCalleNumero = document.getElementById("CalleNumero").value;
	if (vCalleNumero == "" || isNaN(vCalleNumero)) {
		todoBien = false;
		document.getElementById("calleNumero").className = "textMal";
	}
	else {
		document.getElementById("calleNumero").className = "textBien";
	}
...
function validaDatosPagoYEnviaCarro(elEvento) {
	alert("Gracias por su compra, pronto se enviara su pedido.");
